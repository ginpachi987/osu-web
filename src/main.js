...
function toMenu() {
  if (!beatmap) return
  beatmapList.style.display = 'block'
  //startButton.show()
  beatmap.audio.stop()
  beatmap = null
...
function cursorUpdate() {
  ...
  cursor.style.transform = `rotate(${cursorAngle}rad)`
}
...
class Beatmap {
  ...
  drawInfo() {
    ...
    this.hitObjects.filter(el => {
      let typeS = el.type === 'S'
      let display = currentTime - el.time < el.duration
...
class HitObject {
  ...
  constructorV7(data) {
    ...
    for (let i = 1; i < data.length - 1; i++) {
      ...
      if (data[i][1]) stops = parseInt(data[i][1])
    }
    let time = parseInt(data[0][2])
    let combo = parseInt(data[0][3])
    let type = data[0][5] == '0' ? 'P' : data[0][5] > 0 ? 'S' : data[0][5]
    let duration = type === 'S' ? parseInt(data[0][5]) - time : points.length > 1 ? points.length * 100 * stops : 0

    if (stops !== 1) console.log(stops, duration)
  ...
  draw(currentTime) {
    ...
    if (this.type == 'S') {
      ...
      stroke(beatmap.colors[this.col])
      strokeWeight(100)
      for (let i = 1; i < this.points.length; i++) {
